...
for (j in 1:length(categories_list)) {
  for (i in j:length(categories_list)) {
    if(steam_games_df %>% 
...
```
I'd need 697 connections to map out categoreis in a network graph
...
for (j in 1:length(genres_list)) {
  for (i in j:length(genres_list)) {
    if(steam_games_df %>% 
       filter(str_detect(genres, genres_list[j]) &
              str_detect(genres, genres_list[i])) %>% 
       count() %>% 
       pull()
       != 0 ) {count_connections = count_connections + 1} 
  }
...
print(count_connections)
```
If I include all genres, I'll need 422 connections
