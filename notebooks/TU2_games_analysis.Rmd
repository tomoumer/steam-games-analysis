...
steam_games_df %>% 
  ...
  arrange(desc(n))
```
Checking out possible combinations for potentially building a network graph.
...
#from = vector('character', 389)
#to = vector('character', 389)
...
```
If I include all genres, I'll need 389 connections.

Time to do the network graph!!

```{r}
# to make the igraph
g <- graph_from_data_frame(relations, directed=FALSE, vertices=apps_count_by_genre)
```
...
network <- plot_ly(x = ~Xn, 
                   y = ~Yn, 
                   mode = 'markers', 
                   marker = list(size = 100* (apps_count_by_genre %>% pull(num_apps)) / (apps_count_by_genre %>% select(num_apps) %>% sum())),
                   #text = vs$label, 
                   text = paste('Genre:', unique_names, '<br>N. Games:', apps_count_by_genre$num_apps),
                   hoverinfo = 'text'
                   )
...
for(i in 1:Ne) {
  ...
  edge_shape = list(
    ...
    line = list(color = "#030303",
                width = 30 * (relations %>% pull(num_connections))[i] / (relations %>% 
  select('num_connections') %>% sum())
                ),
    x0 = Xn[v0],
    y0 = Yn[v0],
    x1 = Xn[v1],
...
```{r}
steam_games_df %>% 
  filter(str_detect(genres, '360 Video'))
```
